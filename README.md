...
코딩 에이전트는 많지만, 대부분 "코드를 대신 짜주는 도구"에 머뭅니다.
...
| **프로젝트 프로파일** | 스택·규약·환경 제약은 에이전트 정의가 아닌 프로파일에 — 같은 `.claude/` 세트를 어떤 스택의 프로젝트에도 재사용 |
...
```text
# 3. Claude 에게 지시
> 이 문서의 §0 부트스트랩 프로토콜대로 프로젝트를 구성해
```

Claude가 환경 점검 → 질문(플랫폼·리포지토리·보드) → 26개 파일 생성 →
...
```text
/kickoff   프로파일 인터뷰 → 헌장 승인 → 요구사항 인테이크 템플릿 발급
   ⏸️      (사람이 인테이크 작성 — 비동기, 현업 협의 가능)
/intake    회수 → planner 정제 → RFI 질문 루프 → 베이스라인 → 마일스톤 분할 → 백로그 생성
